...
wget https://github.com/uav4geo/OpenPointClass/releases/download/latest/opc-linux-x64.tar.gz
tar -xzvf opc-linux-x64.tar.gz
...
SCALES=$(jq '.scales' settings.json)
RESOLUTION=$(jq '.resolution' settings.json)
RADIUS=$(jq '.radius' settings.json)
...
# Steps 8-10: Call compare.py with the list created in step 9
python compare.py

# Step 11: Create a new comment on the PR with the output of compare.py from step 10
PR_COMMENT=$(python compare.py)
